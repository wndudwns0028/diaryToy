node {
     ...
     stage('Push image') {
         docker.withRegistry('https://registry.hub.docker.com', 'sub_jun_diary') {
...
#변경점
     ...
     stage('Push image') {
         docker.withRegistry('https://registry.hub.docker.com', 'sub_jun_diary') #업로드할 레지스트리 정보, Jenkins Credentials ID {
